document.addEventListener("DOMContentLoaded", function () {
  ...
  if (calculateGPABtn) {
    calculateGPABtn.addEventListener("click", function () {
      ...
      const totalCreditHours = parseFloat(
        document.getElementById("totalCreditHours").value,
      );
      ...
      if (
        isNaN(totalCreditHours) ||
        totalCreditHours < 0 ||
        totalCreditHours > 999
      ) {
        errorElement.textContent =
          "Total credit hours must be between 0 and 999.";
        errorElement.style.display = "block";
        resultsElement.style.display = "none";
        return;
      }
      ...
      for (const course of courseInputs) {
        ...
        const gradeValue = parseFloat(
          course.querySelector(".expected-grade").value,
        );

        if (isNaN(creditHours) || creditHours <= 0 || creditHours > 5) {
          errorElement.textContent =
            "Credit hours must be between 1 and 5 for each course.";
          errorElement.style.display = "block";
          resultsElement.style.display = "none";
          return;
        }
      ...
      // Calculate GPAs
      const semesterGPA =
        totalSemesterHours > 0 ? totalSemesterPoints / totalSemesterHours : 0;
  ...
  if (calculateGoalGPABtn) {
    calculateGoalGPABtn.addEventListener("click", function () {
      const currentGPA = parseFloat(
        document.getElementById("goalCurrentGPA").value,
      );
      const totalCreditHours = parseFloat(
        document.getElementById("goalTotalHours").value,
      );
      const targetGPA = parseFloat(document.getElementById("targetGPA").value);
      const plannedCreditHours = parseFloat(
        document.getElementById("plannedCreditHours").value,
      );
      ...
      if (
        isNaN(plannedCreditHours) ||
        plannedCreditHours <= 0 ||
        plannedCreditHours > 999
      ) {
        errorElement.textContent =
          "Planned credit hours must be between 1 and 999.";
        errorElement.style.display = "block";
        resultElement.style.display = "none";
        return;
      }
