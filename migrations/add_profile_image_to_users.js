...
async function migrateTables() {
  try {
    console.log(
      "Starting migration to add profile_image column to users table...",
    );

    // Check if column already exists
    const [columns] = await db.query("SHOW COLUMNS FROM users LIKE ?", [
      "profile_image",
    ]);
    ...
    if (columns.length === 0) {
      ...
      await db.query(`
            ...
            `);
      console.log(
        "Migration successful: Added profile_image column to users table",
      );
    } else {
