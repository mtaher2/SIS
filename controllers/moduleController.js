...
const checkModuleAuthorization = async (moduleId, instructorId) => {
  try {
    ...
    const isInstructorAssigned = instructors.some(
      (instructor) => instructor.user_id == instructorId,
    );
...
exports.getModules = async (req, res) => {
  try {
    ...
    if (!isInstructorAssigned) {
      req.flash(
        "error_msg",
        "You are not authorized to view modules for this course",
      );
...
exports.updateModule = async (req, res) => {
  try {
    ...
    if (!isInstructorAssigned) {
      req.flash(
        "error_msg",
        "You are not authorized to update modules for this course",
      );
      return res.redirect("/instructor/courses");
    }
...
exports.toggleModulePublish = async (req, res) => {
  try {
    ...
    req.flash(
      "success_msg",
      `Module ${
        moduleData.published ? "published" : "unpublished"
      } successfully`,
    );
    res.redirect(`/instructor/courses/${courseId}/modules`);
  } catch (error) {
...
exports.deleteModule = async (req, res) => {
  try {
    ...
    if (!isInstructorAssigned) {
      req.flash(
        "error_msg",
        "You are not authorized to delete modules for this course",
      );
      return res.redirect("/instructor/courses");
    }
...
exports.updatePage = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      req.flash("error_msg", errors.array()[0].msg);
      return res.redirect(
        `/instructor/modules/${moduleId}/pages/${pageId}/edit`,
      );
    }
    ...
    res.redirect(
      `/instructor/modules/${req.params.moduleId}/pages/${req.params.pageId}/edit`,
    );
  }
};
...
exports.updateQuiz = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      req.flash("error_msg", errors.array()[0].msg);
      return res.redirect(
        `/instructor/modules/${moduleId}/quizzes/${quizId}/edit`,
      );
    }
    ...
    res.redirect(
      `/instructor/modules/${req.params.moduleId}/quizzes/${req.params.quizId}/edit`,
    );
  }
};
...
exports.createQuizQuestion = async (req, res) => {
  try {
    ...
    if (
      ...
      questionData.question_text.trim() === ""
    ) {
      return res.status(400).json({
        success: false,
    ...
    if (
      ...
      if (!questionData.options || questionData.options.length === 0) {
        return res.status(400).json({
          success: false,
          message:
            "Short answer questions must have at least one accepted answer",
        });
      }
      ...
      if (!questionData.options || questionData.options.length < 4) {
        return res.status(400).json({
          success: false,
          message:
            "Matching questions must have at least 2 pairs (4 items total)",
        });
      }

      // Validate matching pairs have both sides
      const hasBothSides = questionData.options.every(
        (opt) =>
      ...
      if (!hasBothSides) {
        ...
        return res.status(400).json({
          success: false,
          message:
            "Matching questions must include properly formatted pairs with left and right sides",
        });
      }
    ...
    if (req.xhr || req.headers.accept.includes("json")) {
      ...
      res.redirect(
        `/instructor/modules/${moduleId}/quizzes/${quizId}/questions`,
      );
    }
  } catch (error) {
    ...
    if (req.xhr || req.headers.accept.includes("json")) {
      ...
      res.redirect(
        `/instructor/modules/${req.params.moduleId}/quizzes/${req.params.quizId}/questions`,
      );
    }
  }
};
...
exports.getAssignments = async (req, res) => {
  try {
    ...
    req.flash(
      "error_msg",
      "An error occurred while retrieving module assignments",
    );
    res.redirect("/instructor/courses");
  }
};
...
exports.createAssignment = async (req, res) => {
  try {
    ...
    res.redirect(
      `/instructor/modules/${req.params.moduleId}/assignments/create`,
    );
  }
};
...
exports.updateAssignment = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      req.flash("error_msg", errors.array()[0].msg);
      return res.redirect(
        `/instructor/modules/${moduleId}/assignments/${assignmentId}/edit`,
      );
    }
    ...
    res.redirect(
      `/instructor/modules/${req.params.moduleId}/assignments/${req.params.assignmentId}/edit`,
    );
  }
};
...
exports.getSubmission = async (req, res) => {
  try {
    ...
    if (submissionRows.length === 0) {
      ...
      return res.redirect(
        `/instructor/modules/${moduleId}/assignments/${assignmentId}/submissions`,
      );
    }
    ...
    res.redirect(
      `/instructor/modules/${req.params.moduleId}/assignments/${req.params.assignmentId}/submissions`,
    );
  }
};
...
exports.gradeSubmission = async (req, res) => {
  try {
    ...
    if (isNaN(parsedScore) || parsedScore < 0 || parsedScore > maxScore) {
      ...
      return res.redirect(
        `/instructor/modules/${moduleId}/assignments/${assignmentId}/submissions/${submissionId}`,
      );
    }
    ...
    await Instructor.calculateStudentGrades(
      submission[0].student_id,
      submission[0].course_id,
    );
