...
exports.postRegister = async (req, res) => {
    try {
        ...
        const { username, email, password, first_name, last_name, role, student_id, department } = req.body;
        ...
        if (userData.role === 'student') {
            const studentData = {
                user_id: userId,
                student_id: student_id || `STU${Math.floor(10000 + Math.random() * 90000)}`,
...
exports.postForgotPassword = async (req, res) => {
    try {
        ...
        req.flash('success_msg', 'If an account exists with that email, a password reset link will be sent.');
        res.redirect('/auth/login');
    } catch (error) {
